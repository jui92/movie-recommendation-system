# show_st.py (patched loader: prefer legacy H5; fallback to Keras3 weights)
...
def load_data():
    ...
    if not loaded and keras3_w.exists():
        try:
            model.load_weights(str(keras3_w))  # Keras3 weights (no by_name support)
            loaded = True
        except Exception as e:
            st.warning(f'Keras3 weights 로드 실패: {e}')
    if not loaded:
        st.warning('가중치를 로드하지 못했습니다. train_autoint_optimized.py로 학습 후 다시 시도하세요.')
...
def get_recom(uid, user_non_seen_dict, r_year, r_month, topk=20):
    ...
    if 'movie_id' in encs:
        ...
        inv_ids = enc_ids; movies_key = movies_df['movie_id']
    mask = movies_key.isin(pd.Series(inv_ids)); out = movies_df.loc[mask].copy()
...
def evaluate_model_sample(users_sample, k=10, max_items_per_user=300):
    ...
    rows = []; users_sample = list(pd.unique(pd.Series(users_sample)))
    for u in users_sample:
        ...
        try:
            ...
            auc = np.nan
        order = np.argsort(-scores)[:k]; p_at_k = float(np.mean(y_true[order])) if len(order)>0 else np.nan
