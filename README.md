...
| 번호    | 변경 내용 요약                                                                                               | 분류                      | 이유                                                         |
| ----- | ------------------------------------------------------------------------------------------------------ | ----------------------- | ---------------------------------------------------------- |
| **①** | 임베딩 크기·어텐션 레이어·헤드 수·DNN 깊이 확장 
(기존): embed_dim=16, att_layer_num=3, att_head_num=2, dnn_hidden_units=[64, 32], dnn_dropout=0.4 → 비교적 경량 설정
(수정): embed_dim=32, att_layer_num=4, att_head_num=4, dnn_hidden_units=[128, 64, 32], dnn_dropout=0.2 → 표현력 강화 + 드롭아웃 완화로 학습 신호 보존을 노림
| **정확도 상승**        | 모델 용량과 표현력을 높여 복잡한 상호작용을 더 잘 학습하게 함.  |
